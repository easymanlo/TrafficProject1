...
//AccumulatorV2<String,String> 第一个string是传进来的字符串，后一个是需要返回的一个个字符串
public class SelfDefineAccumulator extends AccumulatorV2<String,String> {
     ...
     * 卡扣坏的数，卡扣下又几个摄像头是坏的，坏的摄像头的详细信息，就id号
     * @param v  //abnormalMonitorCount=1|abnormalCameraCount=3|abnormalMonitorCameraInfos="0002":07553,07554,07556
     */
    @Override
...
//        System.out.println("myAdd v2="+v2);
        ...
        /*abnormalMonitorCount=1
        abnormalCameraCount=3
        abnormalMonitorCameraInfos="0002":07553,07554,07556
        *abnormalMonitorCount=1|
        * */
        ...
        for (String string : valArr) {
            ...
            if(oldVal != null){
                //只有这个字段是string，所以单独拿出来  不正常的摄像头卡扣信息 是一个字符串类型
                //abnormalMonitorCameraInfos="0002":07553,07554,07556
                if(Constants.FIELD_ABNORMAL_MONITOR_CAMERA_INFOS.equals(field)){
                    //这if在merge的时候才会调用到这个一句
                    if(value.startsWith(" ~")) {
