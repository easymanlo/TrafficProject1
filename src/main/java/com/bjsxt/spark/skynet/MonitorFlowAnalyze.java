 ...
 *  4.topN 卡口的车流量具体信息存库
 *  5.获取高速通过的TOPN卡扣（carcount，minitor_id）
...
//				 sc.accumulator("", new MonitorAndCameraStateAccumulator());
		...
		spark.sparkContext().register(monitorAndCameraStateAccumulator,"SelfAccumulator");
		 ...
		 /**
		  * 将row类型的RDD 转换成kv格式的RDD   k:monitor_id  v:row（卡扣信息，row）
		  */
		 ...
		 */
          ...
          * topNMonitor2CarFlow(minitor_id,car_count)
          * monitor2DetailRDD(mingitor,row)  1,
		  */
	 	 getTopNDetails(taskId,topNMonitor2CarFlow,monitor2DetailRDD);
	 	 
	 	 /**
	 	   * 获取车辆高速通过的TOPN卡扣
          * 自定义了一个sortbykey，实习了comparable接口，中的implement的方法
	 	   */ 
...
//	                System.out.println("使用arrlist排序");
	 ...
	 * @param area2
	 */
	private static void CarPeng(SparkSession spark,
...
//					areaId = row.getString(7);
	 ...
	 */
	private static void saveMonitorState(Long taskId,
...
//										+Constants.FIELD_ABNORMAL_MONITOR_CAMERA_INFOS+"="+monitorId+":"+abnormalCameraInfos.toString().substring(1));
						...
						String standardCameraIds =
								//  转入一个值，以|为切分，想要谁的值（Constants.FIELD_CAMERA_IDS）
									StringUtils.getFieldFromConcatString(standardCameraInfos, "\\|", Constants.FIELD_CAMERA_IDS);
